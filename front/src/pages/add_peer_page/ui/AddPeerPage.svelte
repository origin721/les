<script lang="ts">
    ...
    function addPeer(e: Event) {
      e.preventDefault();

    const formData = new FormData(e.target as HTMLFormElement);

    const peerPeerId = formData.get('peerPeerId');
    //const password = formData.get('password');

    console.log('peerPeerId:', peerPeerId);
...
<div>
  ...
  <form on:submit={addPeer}>
    <label>
      <span>peerId пира</span>
      <input name="peerPeerId"/>
    </label>
