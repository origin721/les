...
import { put_friends, type FriendEntityPut } from "../../indexdb/main_les_store_v1/entities/friends/put_friends";
...
export const friends_service = {
  async add(list: FriendEntity[], myAccId?: string): Promise<FriendEntityFull[]> {
    devLog('🔄 friends_service.add starting with list:', list, 'myAccId:', myAccId);
    ...
    try {
      ...
      if (Object.keys(back_store.accounts_by_id).length === 0) {
        ...
        devLog('✅ Аккаунты загружены в back_store:', Object.keys(back_store.accounts_by_id));
      }
      ...
      prodInfo(`✅ friends_service.add add_friend завершен за ${addFriendDuration} мс, получаем список...`);
      
      const getListStartTime = Date.now();
      const result = await friends_service.getList();
      const getListDuration = Date.now() - getListStartTime;
      
      prodInfo(`✅ friends_service.add getList завершен за ${getListDuration} мс`);
      prodInfo('✅ friends_service.add завершен, результат:', result);
      return result;
