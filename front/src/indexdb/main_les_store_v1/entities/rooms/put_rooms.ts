...
export function put_rooms(
  ...
  devDB(
    "🔄 Обновление комнат начато, количество:",
    update_list.length,
    "explicitMyAccId:",
    explicitMyAccId,
  );
        ...
        try {
          ...
          for (let item of update_list) {
            ...
            if (cachedAccount && cachedAccount.pass) {
              ...
              devAuth(
                "🚀 Пароль получен из back_store для аккаунта:",
                accountId,
              );
            } else {
          ...
          transaction.onabort = function (event) {
            ...
            const error = new Error(
              `IndexDB transaction aborted: ${JSON.stringify(event)}`,
            );
            mRej(error);
            rej(error);
          };
