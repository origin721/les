...
export async function migrationData(context: MigrationContext): Promise<void> {
  ...
  prodInfo(`🔄 Выполняем миграцию данных 2 для пользователя: ${currentUser.id}`);
    ...
    try {
      ...
      getAllRequest.onsuccess = async function() {
        try {
          ...
          const userAccountRecords = allAccountRecords.filter(record => record.id === currentUser.id);
          ...
          prodInfo(`📋 Найдено ${userAccountRecords.length} записей аккаунтов для миграции`);
          
          let migratedCount = 0;
          
          for (const record of userAccountRecords) {
            devMigration(`🔄 Мигрируем аккаунт ${record.id}`);
            ...
            if (accountData.version !== undefined) {
              devMigration(`⏭️ Аккаунт ${record.id} уже содержит поле version: ${accountData.version}, пропускаем`);
              continue;
            }
            ...
            devMigration(`✅ Аккаунт ${record.id} обновлен с version: ${ACCOUNTS_VERSION}`);
          }
          
          transaction.oncomplete = function() {
            prodInfo(`✅ Миграция данных 2 завершена для пользователя ${currentUser.id}. Обновлено: ${migratedCount} аккаунтов`);
            resolve();
          };
